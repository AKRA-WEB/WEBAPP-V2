...
// malformed strings.
function classifyDateField(raw) {
  const value = (raw ?? "").trim();
...
// fields.
function usableBillRefUid(raw) {
  const ref = (raw ?? "").trim();
  return ref && ref !== "DIRECT" ? ref : "";
}

function billGroupKey(row) {
...
const LIFT_FEE_CURRENT_RE = /\[ค่าลิฟท์\s*(\d+)?\s*รอบ\s*(จ่ายสด|เชื่อ)\]/;
const LIFT_FEE_LEGACY_RE = /\[ค่าลิฟท์:\s*(จ่ายสด|เชื่อ)\]/;
const EXTRA_ITEM_TAG = "[นอกบิล/ของแถม]";
