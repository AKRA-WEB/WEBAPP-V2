...
production files, or secrets changed. `V2-0046` (2026-06-24, Complete) plans
and now delivers the operational-readiness gate requested before PR/PO/GR
write workflow: tasks 1-5 added `docs/operations/environment-matrix.md`,
...
`docs/migration/cutover-checklist.md`. All five are documentation/planning
only — no monitoring tool installed, no restore drill run, no rollback owner
named yet (tracked as Open Questions in each doc); that tooling/verification
work is the explicitly deferred task 7. ADR `0025` accepts the gate: `V2-0044`
staging import/read-only validation may continue, but transactional PR/PO/GR
writes should wait until this readiness package is reviewed/accepted (not
necessarily fully tooled — see
`docs/operations/pr-po-gr-readiness-gates.md`'s distinction); production
cutover requires implemented and verified readiness checks, a stricter bar.
...
`V2-0048` (2026-06-25, Complete; flowchart revision 2026-06-26) produced an
import-ready Figma/FigJam workflow board from
`docs/architecture/app-flow-diagrams.md`, adjusted with the latest plan-board
status where the Mermaid source was stale (`V2-0047` means PO/GR now have
read-only runtime UI). After review feedback that the first two SVG versions
were too compact / too list-like, the generator now emits a proper flowchart
diagram (`3600 x 8058`) with 101 flowchart nodes, 101 labeled edges, diamond
decision nodes, and explicit red reject/blocked outcomes (for example,
`Supervisor review -> Reject -> Status: rejected / closed -> No PO is
created`). Direct remote FigJam writing was not possible in this session: no
Figma/FigJam write tool is exposed, and the supplied
`figma.com/board/new...` URL is a new-board URL, not an editable node link.
...
adds `/purchasing/pr/new` and `/purchasing/pr/[id]`, and extends
`/purchasing` with a PR section plus writer-only "New PR" action while keeping
the existing PO list. The staging migration was applied, schema verification
...
38. `V2-0046` - Operational readiness before PR/PO/GR writes
   - Status: Complete on 2026-06-24 — tasks 1-6 done (documentation/planning
     only; task 7, the optional tooling-implementation slice, is not started).
   - Outcome: delivered the readiness package required before transactional
     PR/PO/GR write workflow: `docs/operations/environment-matrix.md`,
     `docs/operations/monitoring-observability-plan.md`,
     `docs/operations/backup-dr-plan.md`,
     `docs/operations/module-rollback-runbook.md`, and
     `docs/operations/pr-po-gr-readiness-gates.md` (tasks 1-5); linked all of
     them from `docs/migration/cutover-checklist.md` (task 6). Each doc names
     its own real current gaps as Open Questions rather than inventing
     numbers: no monitoring tool installed, no restore drill run, no DR
     profile or rollback owner chosen yet, only one Supabase project exists
     (staging) with no separate Production project. ADR `0025` accepts the
     gate: `V2-0044` import/read-only validation may proceed (already done),
     and per `pr-po-gr-readiness-gates.md`, PR/PO/GR write-workflow
     *implementation* against Staging may now start once this package is
     reviewed/accepted — it does not require the Open Questions to be
     answered first, only acknowledged. Production cutover keeps the
     stricter bar: readiness checks must be implemented and verified (real
     drill, real alert, named owner), not just documented.
   - Next action: user reviews/accepts this readiness package, then PR/PO/GR
     write-workflow implementation (e.g. `V2-0049`+) can be planned. Task 7
     (installing actual monitoring tooling, running a real restore drill) can
     happen anytime as a separate slice and is not a hard prerequisite for
     starting write-workflow implementation in Staging.
...
40. `V2-0048` - FigJam app workflow board
   - Status: Complete on 2026-06-25; flowchart revision on 2026-06-26.
   - Outcome: added an import-ready SVG workflow diagram at
     `docs/figma/akra-v2-app-workflow-board.svg`, generated by
     `scripts/generate-figjam-workflow-board.mjs`, plus import instructions
     in `docs/figma/README.md`. Board covers Main/Auth, Picking, PR, PO, GR,
     Warehouse, Returns, and KPI, with status coloring for implemented,
     read-only, planned/write-gated, placeholder, and low-confidence flows.
     The latest 2026-06-26 revision changes it from a step-list diagram into
     a proper Figma/FigJam-editable flowchart (`3600 x 8058`) with 101
     flowchart nodes, 101 labeled edges, decision diamonds, and explicit
     reject destinations for PR/PO/Returns.
     Direct remote FigJam write was not available from this session.
   - Next action: open the user's FigJam board and drag in the SVG.
   - File: `docs/plans/V2-0048-figjam-app-workflow-board.md`
41. `V2-0049` - PR create write slice
   - Status: Implementation complete and staging-verified on 2026-06-26;
     signed-in browser UAT still pending before any production cutover.
   - Outcome: added `supabase/migrations/20260626071939_pr_create_write_slice.sql`
     with `public.create_purchase_requisition(...)`, a default
     `SECURITY INVOKER` service-role-only RPC that atomically creates one PR
     header, its lines, and a `pr_created` event. Added Purchasing PR
     reference data, create server action, multi-line PR form, PR list/detail
     read model helpers, `/purchasing/pr/new`, `/purchasing/pr/[id]`, and a
     writer-only "New PR" action on `/purchasing`.
   - Verification: `npm run check:migrations`, staging
     `npm run db:apply-migrations -- 20260626071939_pr_create_write_slice.sql`,
     `npm run db:verify-staging-schema`, `npm run lint` (only pre-existing
     FigJam warnings), `npm run typecheck`, and `npm run build` passed. A
     transaction-wrapped direct RPC smoke test created and verified one PR
     header/line/event then rolled back. Production-server HTTP smoke for
     `/purchasing/pr/new` returned 200/content-check true; `next dev` remains
     blocked by sandbox `spawn EPERM`.
